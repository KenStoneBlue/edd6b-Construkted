...
class EDD_CJS_Scripts {
    ...
    if( is_singular('download') && !empty($edd_cjs_options['edd_cjs_cesiumjs_token_key']) ) {
      ...
      if( !empty($download_asset_id) || !empty($download_asset_url) ){
    		...
    		wp_enqueue_style( 'edd-cjs-cesium-widgets-style',  'https://cesiumjs.org/releases/1.56.1/Build/Cesium/Widgets/widgets.css', array(), EDD_CJS_LIB_VER );
        ...
        wp_enqueue_script('edd-cjs-cesium-script', 'https://cesiumjs.org/releases/1.56.1/Build/Cesium/Cesium.js', array('jquery'), EDD_CJS_LIB_VER, true);
